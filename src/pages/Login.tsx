...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!user) {
        console.log('Login: User cancelled or closed login popup');
        setLoading(false);
        return;
      }
      ...
      if (error.code === 'auth/popup-blocked') {
        ...
        setError('로그인 중 오류가 발생했습니다. 브라우저의 제3자 쿠키 설정을 확인하거나, 우측 상단의 "새 탭에서 열기" 버튼을 눌러 새 창에서 로그인해 주세요.');
      }
    } finally {
      setLoading(false);
    }
        ...
        <Card className="rounded-none border-[#E5E1DA] shadow-sm">
          ...
          <CardContent className="pt-6 space-y-4">
            ...
            <Button 
              ...
              disabled={loading}
              className="w-full bg-white text-[#1A1A1A] border border-[#E5E1DA] hover:bg-[#FDFCFB] py-7 text-lg font-medium flex items-center justify-center gap-3 rounded-none disabled:opacity-50"
            >
              {loading ? (
                <div className="w-6 h-6 border-2 border-[#8B7E74] border-t-transparent rounded-full animate-spin"></div>
              ) : (
                <>
